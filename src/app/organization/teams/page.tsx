...
export default function TeamsPage() {
  ...
  const handleEdit = () => {
    ...
    const teamToUpdate = teams.find((t) => t.id === selectedTeam?.id);
    if (!teamToUpdate) {
    ...
    useAuditStore
      ...
      .addLog({
        actor: currentUser?.email || "unknown",
        action: "edited team",
        target: `${selectedTeam?.name} → ${teamName}`,
        orgId: orgId || "unknown",
        timestamp: new Date().toISOString(),
      });
  ...
  const filteredTeams = teams.filter((team) => {
    ...
    const memberMatch = team.members.some((m: string) =>
